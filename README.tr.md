...
Sihirbaz, çalışmaya hazır bir `.env` üretir (güçlü gizli anahtarları sizin
yerinize oluşturur) ve seçiminize göre sonraki adımları ekrana yazar.
...
2. Vercel panelinden Environment Variables ekleyin:
   - **Firebase modu:** `DATA_BACKEND=firebase`, `FIREBASE_SERVICE_ACCOUNT_BASE64=<...>`
   - **Postgres modu:** `DATA_BACKEND=postgres`, `DATABASE_URL=<yönetilen Postgres URL>`,
     `PGSSL=true`, `JWT_SECRET=<openssl rand -hex 32>`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`
     (serverless ortamda yerel disk yoktur — [Neon](https://neon.tech),
     [Supabase](https://supabase.com) veya RDS gibi yönetilen bir Postgres kullanın;
     mümkünse *pooled* bağlantı adresini tercih edin)
...
`Dockerfile`'ı deploy edin, bir Postgres eklentisi bağlayın, compose ile aynı
env değişkenlerini girin. Başka bir şey gerekmez — kurulum açılışta otomatiktir.
